...
for direction_name, _, filenames in os.walk('input'):
    for filename in filenames:
        print(os.path.join(direction_name, filename))
print(os.listdir('input'))

# Opening file for reading in binary mode
with open('input/Dataset/data2.pickle', 'rb') as f:
...
# Preparing function for plotting set of examples
...
# Trying one model with filter size 3x3
model = Sequential()
...
# Different models with different filter dimensions
...
# Training set of models of CNN with Keras, with different filter dimensions
annealer = LearningRateScheduler(lambda x: 1e-3 * 0.95 ** (x + epochs))
epochs = 5
...
plt.legend(
    ['filter 31', 'filter 25', 'filter 23', 'filter 19', 'filter 15', 'filter 13', 'filter 9', 'filter 5', 'filter 3'],
...
# Calculating accuracy with testing the dataset
for i in range(len(model)):
...
# Getting labels
labels = label_text('input/Dataset/label_names.csv')
